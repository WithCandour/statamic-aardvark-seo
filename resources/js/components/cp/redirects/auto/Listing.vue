<template>
    <data-list :columns="columns" :rows="rows" v-if="rows.length && enabled">
    ...
    <div v-else class="md:pt-8 max-w-2xl mx-auto">
        <div class="w-full md:w-1/2">
            <h1 class="mb-4" v-if="enabled">Automatic redirects</h1>
            <p class="text-grey-70 leading-normal mb-4 text-lg antialiased">
                <span v-if="enabled">
                    Auto redirects will be generated when changes are made to your page urls.
                </span>
                <span v-else>
                    Auto redirects are not enabled on your Statamic site.
                </span>
            </p>
        </div>
    </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                enabled: this.autoRedirectsEnabled
            }
        },
